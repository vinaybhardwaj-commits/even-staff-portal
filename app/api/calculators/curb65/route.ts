...
type Body = Partial<Curb65Inputs> & { parent_trace_id?: string; idempotency_key?: string };
...
const KEYS: (keyof Curb65Inputs)[] = ['confusion', 'urea_high', 'rr_ge_30', 'bp_low', 'age_ge_65'];
...
export async function POST(req: NextRequest) {
  ...
  const inputs = body as Curb65Inputs;
  const det = computeCurb65(inputs);

  const traceId = await startTrace('curb65', inputs, 1, { score: det.score, band: det.band });
