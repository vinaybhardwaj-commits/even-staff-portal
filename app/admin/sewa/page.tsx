  ...
  return (
    <AdminPageChrome title="Sewa — staff complaints">
      <div className="mb-3 flex justify-end">
        <Link href={`/admin/sewa/type-catalog`} className="inline-flex items-center gap-1 text-[11px] text-[var(--color-text-muted)] hover:text-brand">
          <Settings className="w-3.5 h-3.5" /> Type catalog
        </Link>
      </div>
      <AdminSewaClient adminToken={process.env.ADMIN_TOKEN || ''} />
    </AdminPageChrome>
  );
}
